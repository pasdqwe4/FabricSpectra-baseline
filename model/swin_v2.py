...
def compute_mask(L, window_size, shift_size):
    ...
    if (pad_size == 0) or (pad_size + shift_size == window_size):
        ...
        segs = (
            slice(-seg1), slice(-seg1, -window_size), slice(-window_size, -shift_size), slice(-shift_size, None))
    elif pad_size + shift_size < window_size:
        seg1 = int(window_size * 2 - L + shift_size)
        segs = (
            slice(-window_size), slice(-window_size, -seg1), slice(-seg1, -shift_size), slice(-shift_size, None))
    cnt = 0
    for d in segs:
        img_mask[:, d, :] = cnt
        cnt += 1
...
class SwinTransformerBlock_1D(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0:
            ...
            attn_mask = compute_mask(L, self.window_size, self.shift_size)
        else:
            attn_mask = None
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock_1D(dim=dim, input_resolution=input_resolution,
...
class PatchEmbed1D(nn.Module):
    ...
    """
    def __init__(self, seq_len, patch_size=4, in_chans=32, embed_dim=128, norm_layer=None):
        super().__init__()
...
class SwinTransformerV2_1D(nn.Module):
    ...
    def __init__(self, seq_len=500, patch_size=4, in_chans=32, num_classes=9,
        ...
        self.patch_embed = PatchEmbed1D(seq_len=seq_len,
...
def Swinv2_t(**kwargs):
    model = SwinTransformerV2_1D(seq_len=200, patch_size=4, in_chans=2, num_classes=12,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=16, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0]).cuda()
    return model
